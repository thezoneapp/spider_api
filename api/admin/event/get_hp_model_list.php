<?
	...
	*/
	...
	else $search_sql = "and (modelName like '%$searchValue%') ";

	// 전체 데이타 갯수
    $sql = "SELECT idx FROM hp_event_model WHERE idx > 0 $search_sql";
	...
	// 조건에 맞는 데이타 검색 
	$data = array();
    $sql = "SELECT @a:=@a+1 no, idx, channelIdx, modelCode, modelName, useYn 
	        FROM (SELECT idx, channelIdx, modelCode, modelName, useYn 
				  FROM hp_event_model 
