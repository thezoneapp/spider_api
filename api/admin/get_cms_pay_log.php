<?
	...
	*/
    ...
    $sql = "SELECT @a:=@a+1 no, idx, memId, memName, payMonth, payAmount, message, status, adminId, adminName, wdate 
	        FROM ( select idx, memId, memName, payMonth, payAmount, message, status, adminId, adminName, date_format(wdate, '%Y-%m-%d') as wdate 
		           from cms_pay_log 
		           where idx > 0  $search_sql $status_sql $payMonth_sql 
		         ) m, (select @a:= 0) as a 
			ORDER BY no DESC
