<?
	...
	*/
	...
	//$agencyId = "handy";
	//$policyDate = "2111-11-26 10:00";
	//$excelFile = "211126093642.xlsx";
	...
	try {
		...
		for ($i = 3 ; $i <= $maxRow ; $i++) {
			...
			// 공시지원
			$sql = "INSERT INTO hp_agency_commi (agencyId, policyDate, telecom, discountType, modelCode, priceNew, priceMnp, priceChange, wdate)
			    		                 VALUES ('$agencyId', '$policyDate', '$telecom', 'S', '$modelCode', '$priceNew_S', '$priceMnp_S', '$priceChange_S', now())";
			$connect->query($sql);

			// 선택약정
			$sql = "INSERT INTO hp_agency_commi (agencyId, policyDate, telecom, discountType, modelCode, priceNew, priceMnp, priceChange, wdate)
			    		                 VALUES ('$agencyId', '$policyDate', '$telecom', 'C', '$modelCode', '$priceNew_C', '$priceMnp_C', '$priceChange_C', now())";
			$connect->query($sql);
		}
