	...
	if ($result->num_rows > 0) {
		...
		if ($result3->num_rows > 0) {
			...
			if ($channelAssort == "L") { // 링크가입
				...
				sendTalk($groupCode, "HP_01_01", $receiptInfo);
				...
				sendTalk($groupCode, $smsCode, $receiptInfo);
				...
				sendTalk($groupCode, "HP_05_01_01", $receiptInfo);
			}
			...
			sendTalk($groupCode, "HP_02_03", $receiptInfo);

			// 알림톡 전송(관리자)
			sendTalk($groupCode, "HP_03_03_03", $adminInfo);

			// 알림톡 전송(마선빈)
			sendTalk($groupCode, "HP_03_03_03", $admin2Info);

			// 알림톡 전송(운영팀. 박성현과장)
			sendTalk($groupCode, "HP_03_03_03", $admin3Info);

			// 알림톡 전송(드림엠제이. 현정님)
			sendTalk($groupCode, "HP_03_03_03", $admin4Info);
