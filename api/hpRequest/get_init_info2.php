		...
		while($row = mysqli_fetch_array($result)) {
			...
			if ($result2->num_rows > 0) {
				while($row2 = mysqli_fetch_array($result2)) {
					...
					// 수수료 정보
					$commiData = array();
					$sql = "SELECT telecom, assortCode, priceNew, priceMnp, priceChange 
							FROM hp_commi 
					...
					if ($result3->num_rows > 0) {
						while($row3 = mysqli_fetch_array($result3)) {
							$priceNew    = $row3[priceNew];
							$priceMnp    = $row3[priceMnp];
							$priceChange = $row3[priceChange];
							...
							$data_info = array(
								'telecom'      => $row3[telecom],
								'assort'       => $row3[assortCode],
								'priceNew'     => $priceNew,
								'priceMnp'     => $priceMnp,
								'priceChange'  => $priceChange,
							);
							array_push($commiData, $data_info);
						}
					}
